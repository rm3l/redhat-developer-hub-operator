			...
			When(fmt.Sprintf("applying %s (%s)", tt.name, tt.crFilePath), func() {
				...
				It("should handle CR as expected", func() {
					...
					if helper.IsOpenShift() {
						...
						// TODO(rm3l): this is a workaround until this is supported: https://issues.redhat.com/browse/RHIDP-2176
...
`, tt.crName, ingressHost))
							...
							By("ensuring the application is fully reachable", func() {
								Eventually(helper.VerifyBackstageAppAccess, 5*time.Minute, time.Second).
									WithArguments(fmt.Sprintf("https://%s", ingressHost), tt.additionalApiEndpointTests).
									Should(Succeed())
							})
						}
					}
